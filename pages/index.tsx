...
const Home: NextPage = async () => {
  ...
  const sdk = new ThirdwebSDK("mumbai", {
    clientId: "347a1f6357f373a45d910c95ec77b350",
  });
  ...
  const contract = await sdk.getContract("0xCA4b91F7d2B816ee0F5160957dD94B5CC32469A1");
  ...
  useEffect(() => {
    ...
    const noOfUserTickets = totalTickets.reduce(
      (total, ticketsAddress) => (ticketsAddress === address ? total + 1 : total), 0
    );
    ...
    <div className="bg-[#091B18] min-h-screen flex flex-col">
        ...
        <Marquee className="bg-[#0A1F1C] p-5 mb-5" gradient={false} speed={100}>
          <div className="flex space-x-2 mx-10">
            ...
            <h4 className="text-white font-bold">
              Previous winnings: {lastWinnerAmount && ethers.utils.formatEther(lastWinnerAmount?.toString())}{" "}
              {currency}
            </h4>
          </div>
        </Marquee>
      ...
      <footer className="border-t border-emerald-500/20 flex items-center text-white justify-between p-5">
        ...
        <p className="text-xs text-emerald-900 pl-5">
          The information presented is meant for nothing more than learning and entrenment purposes. 
          We are not liable for any losses that are incurred or problems that arise at online casions.
          You are doing so completely and totally at your own risk.
        </p>
      </footer>
    </div>
  );
